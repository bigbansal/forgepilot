...
Manch is an AI coding-agent platform designed to deliver OpenClaw-like capabilities while improving on the areas that usually break in real company environments:
...
Manch should not try to beat OpenClaw by simply adding more agents.
...
> Why Python? Because Manch is primarily an AI orchestration system rather than a classic CRUD backend. Python gives faster iteration, stronger AI integrations, and a better agent tooling ecosystem.

---

## 2. What Manch Is

Manch is a platform that can:
...
## 4.1 Why build Manch if OpenClaw is already strong?
...
Manch should compete on different dimensions:
...
Manch should become the platform teams choose when they want OpenClaw-like power with stronger operational control.

---

## 5. Similarity to OpenClaw

Manch should preserve the best parts of the OpenClaw pattern:
...
These are important because Manch must be designed specifically to avoid them.
...
Manch should follow these non-negotiable principles.
...
Manch should support all of these as first-class concepts.
...
Manch must treat CLI execution as a first-class runtime capability.
...
At minimum, Manch should model the following entities.
...
Manch must use a strict state machine.
...
Manch is successful when it can:
...
Manch should be built as a **controlled OpenClaw-like platform** optimized for enterprise reliability.
...
Manch should not try to be the loudest agent platform.
