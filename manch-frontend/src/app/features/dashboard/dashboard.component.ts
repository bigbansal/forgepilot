...
@Component({
  ...
  template: `
    <section class="page">
      ...
      <fp-spinner *ngIf="loading()" label="Loading dashboard…" />
      ...
      <div class="grid" *ngIf="!loading()">
        ...
        <article class="stat-card">
          ...
          <p class="stat" style="color: var(--c-warning)">{{ pendingApprovalCount() }}</p>
          <p *ngIf="pendingApprovalCount() > 0"><a routerLink="/approvals">Review &rarr;</a></p>
        </article>
      ...
      <article class="panel recent" *ngIf="!loading() && recentTasks().length">
        <h3>Recent Tasks</h3>
        <table>
          <thead>
            <tr>
...
export class DashboardComponent implements OnInit {
  ...
  async refresh(): Promise<void> {
    ...
    try {
      ...
      // Best-effort approval count
      try {
        const res = await this.approvalService.pendingCount();
        this.pendingApprovalCount.set(res.count);
      } catch { /* non-critical */ }
    } catch (error) {
