  ...
  template: `
    <section class="panel">
      ...
      <div class="filters">
        ...
        <select [ngModel]="statusFilter()" (ngModelChange)="statusFilter.set($event)" aria-label="Filter status">
          <option value="ALL">All statuses</option>
          <option *ngFor="let status of statuses" [value]="status">{{ status }}</option>
        </select>

        <select [ngModel]="sortBy()" (ngModelChange)="sortBy.set($event)" aria-label="Sort tasks">
...
export class TaskListComponent implements OnInit {
  ...
  readonly sortBy = signal<'updated_desc' | 'updated_asc' | 'created_desc' | 'created_asc'>('updated_desc');
  ...
  readonly filteredTasks = computed(() => {
    ...
    if (sf !== 'ALL') {
      data = data.filter((task) => task.status === sf);
    }
    ...
    if (sb === 'created_asc') sorted.sort(createdSort);
    if (sb === 'created_desc') sorted.sort((a, b) => createdSort(b, a));
    if (sb === 'updated_asc') sorted.sort(updatedSort);
    if (sb === 'updated_desc') sorted.sort((a, b) => updatedSort(b, a));
