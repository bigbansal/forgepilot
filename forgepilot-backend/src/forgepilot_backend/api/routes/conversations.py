...
from forgepilot_backend.db.models import ChatMessageRecord, ConversationRecord, UserRecord
...
@router.get("")
def list_conversations(current_user: UserRecord = Depends(get_current_user)):
    with SessionLocal() as db:
        rows = (
            db.execute(
                select(ConversationRecord)
                .where(ConversationRecord.user_id == current_user.id)
...
def send_message(
    ...
    with SessionLocal() as db:
        ...
        task = orchestrator.create_task(prompt_with_history, user_id=current_user.id)
    ...
    def _execute_in_background() -> None:
        if request.runner.value == "agent-pipeline":
            ...
            task_result, _session, message, output = orchestrator.start_task(
                task.id, runner=request.runner, approval_mode=request.approval_mode, user_id=current_user.id
            )
            output_text = [
                message,
