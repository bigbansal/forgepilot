...
from forgepilot_backend.agents.base import AgentContext, AgentResult
from forgepilot_backend.agents.registry import get_agent
...
from forgepilot_backend.models import RiskLevel, StepStatus, TaskStatus
...
class PlanExecutionEngine:
    ...
    @staticmethod
    def _emit(event: str, data: dict, user_id: str | None = None) -> None:
        import asyncio

        try:
            loop = asyncio.get_running_loop()
            loop.create_task(event_broker.publish(event, data, user_id=user_id))
        except RuntimeError:
            # Called from a background thread — use the registered main loop
            event_broker.publish_threadsafe(event, data, user_id=user_id)
